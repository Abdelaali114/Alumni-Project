    ...
    const fetchAlbumDetails = async () => {
      try {
        const response = await axios.get(
          `http://192.168.60.254:3001/albums/${albumId}`
        ); // Fetch album by ID
    ...
    <>
      ...
      <Box p={3}>
        ...
        <Grid container spacing={2}>
          {album.images.map((image, index) => (
            <Grid item xs={12} sm={6} md={4} key={index}>
              <CardMedia
                component="img"
                image={`http://192.168.60.254:3001/assets/${image}`}
                alt={`Image ${index + 1}`}
                sx={{
                  width: "100%",
                  height: 200,
        ...
        <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
          ...
          >
            <img
              src={`http://192.168.60.254:3001/assets/${selectedImage}`} // Display selected image
              alt="Selected"
              style={{
                maxWidth: "100%",
                maxHeight: "80vh",
